...
- (void) awakeFromNib {
  ...
  if ([self . wwwFolderName rangeOfString: @"://"] . location != NSNotFound)
          {
...
- (void) configureWindowSize {
  ...
  if ([windowSize isEqualToString: @"fullscreen"])
          {
          _cfgFullScreen = true;
          }
  else
...
- (void) onAppDidFinishLaunching: (NSNotification*) notification {
  ...
  if (_cfgWindowSize . height > 0 && _cfgWindowSize . width > 0)
          {
